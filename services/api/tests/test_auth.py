...
def test_safe_return_path_rejects_external_targets() -> None:
    ...
    assert auth.safe_return_path("https://evil.test") == "/dashboard"
    assert auth.safe_return_path("//evil.test/path") == "/dashboard"
    assert auth.safe_return_path(r"/\evil") == "/dashboard"
...
def test_google_callback_creates_user_session_and_logout(
    ...
    with Session(session_factory.kw["bind"]) as session:
        assert session.scalar(select(User).where(User.google_subject == "google-sub-1"))
...
def test_google_callback_invalid_state_does_not_create_user(
    ...
    assert response.headers["location"] == "/dashboard?auth_error=invalid_state"
